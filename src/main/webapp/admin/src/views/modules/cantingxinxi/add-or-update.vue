<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="餐厅类型" prop="cantingleixing">
          <el-select :disabled="ro.cantingleixing" v-model="ruleForm.cantingleixing" placeholder="请选择餐厅类型">
            <el-option
                v-for="(item,index) in cantingleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  data() {
	...
	price : false,
      ...
      rules: {
          ...
          cantingtupian: [
                { required: true, message: '餐厅图片不能为空', trigger: 'blur' },
          ],
          caipinmingcheng: [
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.cantingleixingOptions = "东区餐厅,万人餐厅,学府餐厅".split(',')
            this.$http({
	...
	this.ruleForm.caipinjieshao = this.ruleForm.caipinjieshao.replace(reg,'../../../ssm5gd2j/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
